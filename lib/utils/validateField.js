...
const validateField = (fields, fieldObj, value, model, errorsArray = []) => {
  ...
  if (!value) {
    if (required === false) {
      ...
      errorsArray.push({ _id, errStr: errStr ? T9n.get(errStr) : `${_id} is required` })
      return
    }
  }
  ...
  if (re && !re.test(value)) {
    errorsArray.push({ _id, errStr: errStr ? T9n.get(errStr) : `${value} is not valid as ${_id}` })
    return
  }

  // Validate min length
  if (minLength && minLength > value.length) {
    errorsArray.push({ _id, errStr: errStr ? T9n.get(errStr) : `${_id} length must be at least ${minLength} characters` })
    return
  }

  // Validate max length
  if (maxLength && maxLength < value.length) {
    errorsArray.push({ _id, errStr: errStr ? T9n.get(errStr) : `${_id} length must be no more than ${maxLength} characters` })
    return
  }
