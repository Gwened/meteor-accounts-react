...
class ResendVerification extends Component {
  ...
  render () {
    ...
    return (
      <Fragment>
        ...
        {emailSent && <p className='email-sent'>{texts.info.emailSent || T9n.get(translations.info.emailSent)}</p>}
  ...
  sendVerificationLink = () => {
    ...
    resendVerification(this.state.email, err => {
      if (err) {
        this.setState({ errors: [{ _id: '__globals', errStr: T9n.get(`error.accounts.${err.reason}`) }], emailSent: false })
      } else {
