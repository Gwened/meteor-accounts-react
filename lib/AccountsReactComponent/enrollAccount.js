...
class EnrollAccount extends Component {
  ...
  render () {
    ...
    return (
      <Fragment>
        ...
        {passwordSet && <p>{texts.info.accountEnrolled || T9n.get(translations.info.accountEnrolled)}</p>}
  ...
  onSubmit = () => {
    ...
    resetPassword(this.props.token, password, err => {
      if (err) {
        this.setState({ errors: [{ _id: '__globals', errStr: T9n.get(`error.accounts.${err.reason}`) }], passwordSet: false })
      } else {
