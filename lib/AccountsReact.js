  ...
  constructor () {
    ...
    this.config = {
        ...
        signUp: [
          {
            ...
            maxLength: 22,
            re: regExp.Username,
            errStr: 'error.accounts.User validation failed', // 'Username must be between 4 and 22 characters',
          },
          {
            _id: 'email',
            displayName: 'email', // 'Email',
            placeholder: 'enterEmail', // 'Enter your email',
            re: regExp.Email,
            errStr: 'error.accounts.Invalid email', // 'Please enter a valid email'
          },
          {
            _id: 'password',
            displayName: 'password', // 'Password',
            ...
            minLength: 6,
            maxLength: 32,
            errStr: 'error.accounts.Incorrect password', // 'Please enter a strong password between 6 and 32 characters',
          ...
          },
          {
            ...
            errStr: 'error.pwdsDontMatch', // 'Password doesn\'t match',
            exclude: true,
            autocomplete: 'new-password',
            func: (fields, fieldObj, value, model, errorsArray) => {
              ...
              if (typeof password === 'string') {
                if (!value || (value !== password)) {
                  errorsArray.push({ _id, errStr: T9n.get(errStr) })
                  return
                }
              }
        ...
        forgotPwd: [
          {
            ...
            placeholder: 'enterEmail', // 'Enter your email',
            re: regExp.Email,
            errStr: 'error.accounts.Email doesn\'t match the criteria.', // 'Please enter a valid email'
          }
        ],
      ...
      translations: {
        ...
        errors: {
          loginForbidden: 'error.accounts.Login forbidden', // 'There was a problem with your login',
          captchaVerification: 'error.accounts.User validation failed', // 'There was a problem with the recaptcha verification, please try again',
          userNotFound: 'error.accounts.User not found', // 'User not found',
          userAlreadyVerified: 'error.accounts.Already verified', // 'User already verified!'
        },
        forgotPwdSubmitSuccess: 'info.emailSent', // 'A password reset link has been sent to your email!',
        loginForbiddenMessage: 'error.accounts.Login forbidden', // 'There was a problem with your login'
    },
      ...
      /* texts are empty be default because translations are used. */
      ...
      Accounts.validateLoginAttempt(attempt => {
        if (attempt.error) {
          ...
          if (reason === 'User not found' || reason === 'Incorrect password') {
            throw new Meteor.Error('Login forbidden', 'Login forbidden') // Throw generalized error for failed login attempts
          }
        }
